...
from users.schemas import ShowUser, UserSchema, CreateUser
...
async def _create_new_user(body: CreateUser, db) -> ShowUser:
    async with db.begin():
        userdal = UserDAL(db)
        new_user = await userdal.create_user(username=body.username, 
            email=body.email, 
            hashed_password=hash_password(body.password))
    
    return ShowUser(username=body.username, 
        email=body.email, 
        is_active=True)
