...
from api.v1.actions.post import _create_post, _delete_post, _get_post_by_id, _get_all_posts, _get_user_posts, _update_post
...
@router.get('/{post_id}')
async def get_post_by_id(post_id: int, db: AsyncSession = Depends(get_session), 
    user: Users = Depends(get_current_user)) -> PostScheme:
...
@router.get('/my_posts')
async def get_current_users_posts(db: AsyncSession = Depends(get_session), 
    user: Users = Depends(get_current_user)):
    try:
...
async def create_new_post(body: PostScheme, 
    db: AsyncSession = Depends(get_session), 
    user: Users = Depends(get_current_user)):
    
    try:
       return await _create_post(title=body.header, content = body.content, author = user.username, db = db)
    except IntegrityError as e:
        raise HTTPException(status_code=503, detail=f"Database error: {e}")
...
async def update_post(body: UpdatedPostScheme, db: AsyncSession = Depends(get_session),
    user: Users = Depends(get_current_user)):
    ...
    if post_to_update is None:
        raise HTTPException(
            status_code=404, detail=f"Post not found."
        )
